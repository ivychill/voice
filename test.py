...
test_audio_dir = Path("/home/kcadmin/user/fengchen/voice/tianchi/datasets/test")
...
weights_path = "output/000_ResNet50/fold0/checkpoints/train.74.pth"
...
def prediction_for_clip(audio_id: str,
    ...
    for image, audio_id in progress_bar(loader):
        ...
        # logger.debug(f'whole_size {whole_size}')
        ...
        for batch_i in range(n_iter):
            ...
            with torch.no_grad():
                ...
                # logger.debug(f'proba_sum {proba_sum.size} {proba_sum}')
            proba_total += proba_sum
            # logger.debug(f'proba_total {proba_total.size} {proba_total}')

        proba_average = proba_total/whole_size
        logger.debug(f'proba_average {proba_average.size} {proba_average}')
...
def prediction(test_audio_dir: Path,
    ...
    for audio_id in unique_audio_id:
        ...
        with timer(f"Prediction on {audio_id}", logger):
            prediction_df = prediction_for_clip(audio_id,
                                                  clip=clip,
                                                  model=model,
...
prediction = prediction(test_audio_dir=test_audio_dir,
